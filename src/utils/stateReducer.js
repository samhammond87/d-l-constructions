export default function reducer (state, action) {
	switch(action.type) {
		...
		case 'updateTimesheet': {
			const timesheet = state.timesheets.find((timesheet) => timesheet.id == action.data.id)
			const theRest = state.timesheets.filter((timesheet) => timesheet.id != action.data.id)
